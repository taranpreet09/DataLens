...
// Use sessionStorage so data survives route remounts
function loadSignupData() {
  try {
    const saved = sessionStorage.getItem('datalens_signup');
    if (saved) return JSON.parse(saved);
  } catch {}
  return { email: '', password: '', fullName: '', role: '', organization: '', platform: 'business_intelligence' };
}

function saveSignupData(data) {
  sessionStorage.setItem('datalens_signup', JSON.stringify(data));
}
...
export default function SignupLayout() {
  ...
  const [signupData, setSignupData] = useState(loadSignupData);

  const updateSignupData = useCallback((fields) => {
    setSignupData((prev) => {
  ...
  const clearSignupData = useCallback(() => {
    sessionStorage.removeItem('datalens_signup');
  }, []);
    ...
    <div className="bg-surface text-on-surface font-body selection:bg-primary selection:text-on-primary antialiased h-screen w-screen flex flex-col overflow-hidden relative">
      ...
      <div className="relative z-40 w-full max-w-md mx-auto px-6 shrink-0 mb-4">
        <div className="flex justify-between items-end mb-2">
          <div>
            <h2 className="font-headline font-bold text-xs text-on-surface-variant uppercase tracking-widest">Step {config.step} of 3</h2>
            <p className="font-headline font-extrabold text-xl text-on-surface mt-0.5">{config.label}</p>
          </div>
          <span className="text-primary font-headline font-bold text-sm">{config.percent}%</span>
        </div>
        <div className="h-1 w-full bg-surface-container-highest rounded-full overflow-hidden">
          <motion.div className="h-full bg-primary rounded-full" initial={false} animate={{ width: `${config.percent}%` }} transition={{ duration: 0.6, ease: [0.4, 0, 0.2, 1] }} />
        </div>
      </div>
      <div className="relative z-10 flex-1 overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div key={location.pathname} initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }} transition={{ duration: 0.35, ease: [0.4, 0, 0.2, 1] }} className="h-full overflow-y-auto no-scrollbar">
            <Outlet context={{ signupData, updateSignupData, clearSignupData }} />
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
